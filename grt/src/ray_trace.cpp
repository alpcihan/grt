...
public:
  ...
  void onUIRender() override
  {
    { // Setting menu
      ...
      PropertyEditor::entry("Metallic", [&]
                            { return ImGui::SliderFloat("#1", &m_pushConst.metallic, 0.0F, 1.0F); });
      PropertyEditor::entry("Roughness", [&]
                            { return ImGui::SliderFloat("#1", &m_pushConst.roughness, 0.0F, 1.0F); });
      PropertyEditor::entry("Intensity", [&]
...
private:
  ...
  void createScene()
  {
    ...
    // Meshes
    m_meshes.emplace_back(nvh::createCube(0.01F, 0.01F, 0.01F));
  ...
  void createBottomLevelAS()
  {
    ...
    for (uint32_t p_idx = 0; p_idx < numMeshes; p_idx++)
    {
      ...
      VkAccelerationStructureBuildSizesInfoKHR sizeInfo =
          buildData.finalizeGeometry(m_device, VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_TRACE_BIT_KHR | VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_COMPACTION_BIT_KHR);
  ...
  void createTopLevelAS()
  {
    ...
    tlasInstances.reserve(m_nodes.size()); // Reserve enough space for all instances
    const nvh::Node &node = m_nodes[0];

    GRTModel model("/home/alp/Desktop/grt/grt/src/_data/data.bin", false);

    for (size_t i = 0; i < model.position.size(); i+=3)
    {
      // Create an instance for each node, at a specific position
      // The instance will use the same BLAS as the node's mesh
      VkAccelerationStructureInstanceKHR ray_inst{
          .transform = nvvk::toTransformMatrixKHR(glm::translate(node.localMatrix(), glm::vec3(model.position[i], model.position[i+1], model.position[i+2]))), // Position of the instance
    ...
    nvvk::Buffer scratchBuffer = m_alloc->createBuffer(sizeInfo.buildScratchSize, VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
